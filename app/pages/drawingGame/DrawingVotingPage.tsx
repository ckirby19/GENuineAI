...
export const DrawingVotingPage = (props: Props) => {
            ...
            <CardContent>
              ...
              {!hasVoted ? (
                <RadioGroup value={votedAnswerId?.toString()} onValueChange={(value) => setVotedAnswerId(value)}>
                  {props.answers 
                      .filter(answer => answer.participantId != currentParticipant?.id && answer.drawing != null)
                      .map((answer, index) => (
                    <div key={answer.id} className="flex items-center space-x-2 mb-4">
                      ...
                      <img
                        src={answer.drawing!}
                        alt={`Drawing ${index + 1}`}
                        className="w-full h-auto rounded-lg border-2 transition-all duration-300 peer-checked:border-[hsl(var(--neon-blue))] hover:border-[hsl(var(--neon-purple))] border-transparent"
                      />
